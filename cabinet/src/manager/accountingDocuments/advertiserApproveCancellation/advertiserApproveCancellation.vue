<template>
    <div>
        ...
        <header-doc
                title="Сторно баланса рекламодателя по конверсиям"
                :status="status"
        ...
        <md-card>
            <md-card-content>
                ...
                <div class="md-layout-item doc-ml--8" v-if="!isViewMode">
                    <md-button class="md-raised" @click="loadData" :disabled="showProgressBar">Заполнить по остаткам
                    </md-button>
                </div>
...
<script>
    ...
    export default {
        ...
        data: () => ({
            ...
            editIndexRow: null,
            saveParams: {
                formName: 'common_modules_accounting_documents_models_AdvertiserApproveCancellationForm'
            },
        }),
        computed: {
            id() {
        ...
        methods: {
            ...
            loadData() {
              ...
              if (!!this.advertiser_id === false) {
                  notify('Выберите рекламодателя', 'danger');
                  return;
              }
              this.loadDataForGrid(this.urlLoadData() + '?client_id=' +  this.advertiser_id)
            },
