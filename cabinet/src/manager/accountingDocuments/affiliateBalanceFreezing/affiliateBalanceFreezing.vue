<template>
    <div>
        <breadcrumbs :breadcrumbs="[
                ...
                {url: '/financeAccountingDocuments/affiliateBalanceFreezing/list',title: 'Заморозка баланса вебмастера'},
                {title: isViewMode ? 'Просмотр'
                    : isEditMode ? 'Редактирование' : 'Новый'}
            ]"/>
        <header-doc
                title="Заморозка баланса вебмастера"
                :status="status"
...
<script>
    ...
    export default {
        ...
        created: function () {
            if(this.isViewMode ||this.isEditMode){
                this.appendDocument(
                    {'url': this.urlDocument(this.extractDocumentId()), 'form': this.$v}
                );
            }
        },
        methods: {
